...
tools: pd-tso-bench pd-heartbeat-bench regions-dump stores-dump pd-api-bench pd-stream-bench pd-ut
...
pd-stream-bench:
	cd tools && CGO_ENABLED=0 go build -o $(BUILD_BIN_PATH)/pd-stream-bench ./pd-stream-bench
pd-recover:
...
.PHONY: pd-ctl pd-tso-bench pd-recover pd-analysis pd-heartbeat-bench simulator regions-dump stores-dump pd-api-bench pd-stream-bench pd-ut
